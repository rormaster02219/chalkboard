function openNavMob()
{
if($(".mobileNav > ul").is(":visible"))
			{
			$(".mobileNav .mob").removeClass('active');
			$(".mobileNav > ul").hide();
			//alert('click 2 hide');
			}
			else
			{
			$(".mobileNav .mob").addClass('active');
			$(".mobileNav > ul").show();
			//alert('click 2 show');
			}
			return false;
...
function openNavMob1()
{
			if($(".openNavMob1 ul").is(":visible"))
			{
			$(".mobileNav .mob").removeClass('active1');
			$(".openNavMob1 ul").hide();
			}
			else
			{
			$(".mobileNav .mob").addClass('active1');
			$(".openNavMob1 ul").show();
			}
			return false;
}

function openNavMob3()
{
                        if($(".openNavMob3 ul").is(":visible"))
                        {
                        $(".mobileNav .mob").removeClass('active1');
                        $(".openNavMob3 ul").hide();
                        }
...
function openNavMob2()
{
			if($(".openNavMob2 ul").is(":visible"))
			{
			$(".mobileNav .mob").removeClass('active1');
			$(".openNavMob2 ul").hide();
			}
...
document.getElementsByTagName('head')[0].appendChild(viewPortTag);
		...
		//setScrenSize();
		//setTimeout(function(){setScrenSize();$("section").customScrollbar();}, 2000);
...
$("body").addClass('customBody');
	   ...
	   {
	   setTimeout(function(){setScrenSize();$("section").customScrollbar();}, 300);
	   setTimeout(function(){setScrenSize();$("section").customScrollbar("resize", true);}, 500);
	   }
	   //setTimeout(function(){setScrenSize();$("section").customScrollbar("resize", true);}, 2000);
...
$(document).ready(function(){
			...
			//setTimeout(function(){$(".mobileNav .mob").show();}, 2000);
...
$(document).ready(function() {
    ...
    $("body").on('click', '.login_path', function(){
	//alert('bd');
      $(".loading").show();
      $.ajax( {
        type: "POST",
  ...
  $( document ).ready(function() {
    ...
    $("#submit_subs_form").click(function(){
      ...
      if($("input:radio[name=payment]:checked").val() == "credit"){
        ...
        var student_id = $('#subscription_student_id').val();
        var subject = $('#subscription_subject').val();
        var pin_number = $('#pin_number').val();
        var activity = $('#subscription_activity').val();
        checkValidPin(pin_number,student_id,subject,activity)
      }
    });

    function checkValidPin(pin_number,student_id,subject,activity){

      $(".loading").show();
      var url_pass = '/pins/' + pin_number + "/" + student_id + "/" + subject + "/" + activity
      var resp = "one"
      $.ajax( {
            type: "GET",
            url: url_pass,
            success: function( response ) {
              if(response.val == true){
                $(".loading").hide();
        	$(".subscription_error h5").html(response.msg);
	...
	function setScrenSize()
	{
	   var windowheight = ($(window).height()) - 95 - 97;
	   $("section").height(windowheight);
	   $(".wrapper .centerRight").height(windowheight);
	   //alert("Browser : " + navigator.userAgent + " Top Height : " + ($(".top").height()) + " Bottom Height : " + ($(".bottom").height()));
